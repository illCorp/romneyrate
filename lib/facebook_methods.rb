      ...
      if facebook_client
        ...
        if (you)
          ...
          if (FacebookUser.exists?(:uid => results['uid'].to_s))
            ...
            @u.update_attributes({name:results['name'], image_url: results['pic_square'], raw_json: results.to_json})
            @u.reload
          else
  ...
  def facebook_friend_likes
    ...
    if user
      begin
        ...
        query = "SELECT uid, first_name, last_name, pic_big, pic_square, name FROM user WHERE uid IN(
        ...
        friends = results.collect do |row|
          if (FacebookUser.exists?(:uid=>row['uid'].to_s))
            ...
            u.update_attributes({name:row['name'], image_url: row['pic_square'], raw_json: row.to_json})
            u.reload
          else
            u = FacebookUser.new({uid: row['uid'].to_s, name:row['name'], image_url: row['pic_square'], raw_json: row.to_json})
            u.save
          end
          u
        end
